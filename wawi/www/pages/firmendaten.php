...
class Firmendaten  {
  ...
  function Firmendaten($app) {
	...
	{
		// 2. = to
		if($this->app->erp->MailSend($this->app->erp->GetFirmaMail(),$this->app->erp->GetFirmaName(),$this->app->erp->GetFirmaMail(),$this->app->erp->GetFirmaName(),"WaWision: Testmail","Dies ist eine Testmail"))
		{
 ...
 */     
  ...
  $data[projekt] = $this->app->DB->Select("SELECT id FROM projekt WHERE abkuerzung='{$data[projekt]}' LIMIT 1");
	...
	$this->app->DB->Update("UPDATE firmendaten SET absender='{$data[absender]}', 
