...
//    $this->app->Tpl->Add(TABS,"<li><a href=\"index.php?module=zeiterfassung&action=list\">Zur&uuml;ck zur &Uuml;bersicht</a></li>");
  ...
  function ZeiterfassungManuell($adr_id)
  {
    ...
    $this->app->Tpl->Set(SUBSUBHEADING, "Zeit erfassen");
		...
		{
			$tmp = $this->app->DB->SelectArr("SELECT *,DATE_FORMAT(von,'%H:%i') as von, DATE_FORMAT(bis,'%H:%i') as bis, DATE_FORMAT(von,'%d.%m.%Y') as datum FROM zeiterfassung WHERE  id='$id' LIMIT 1");
    ...
    if($paketauswahl == 0){
			if(($aufgabe!="") && ($this->app->Secure->GetPOST("vonZeit")!="") && ($this->app->Secure->GetPOST("bisZeit")!="") && ($datum!="")){	
			{
		  	...
		  	$this->app->erp->AddArbeitszeit($adr_id, $vonZeit, $bisZeit, $aufgabe, $beschreibung, $ort, $projekt, 0,$art,$adresse_kunde,$abrechnen,$verrechnungsart,$kostenstelle,$abgrechnet,$gps);
	  	...
	  	$this->app->Tpl->Set(MESSAGE,"<div class=\"error\">Fehler! Die Felder T&auml;tigkeit, Von, Bis, Am, m&uuml;ssen ausgef&uuml;llt sein</div>");
    		...
    		$kunde = $this->app->DB->Select("SELECT kunde FROM projekt WHERE id='$projekt' LIMIT 1");
	  		...
	  		$this->app->erp->AddArbeitszeit($adr_id, $vonZeit, $bisZeit, $aufgabe, $beschreibung, $ort, $projekt, $paketauswahl,$art,$adresse_kunde,$abrechnen,$verrechnungsart,$kostenstelle,$abgerechnet,$gps); 
	  		$this->app->Tpl->Set(MESSAGE,"<div class=\"success\">T&auml;tigkeit erfolgreich gebucht</div>");
	}else{
...
//echo $datum;
    		...
    		if($paketauswahl == 0)
	  		$this->app->erp->UpdateArbeitszeit($id,$adr_id, $vonZeit, $bisZeit, $aufgabe, $beschreibung, $ort, $projekt, $paketauswahl,$art,$adresse_kunde,$abrechnen,$verrechnungsart,$kostenstelle,$abgerechnet,$gps); 
				else
	  		$this->app->erp->UpdateArbeitszeit($id,$adr_id, $vonZeit, $bisZeit, $aufgabe, $beschreibung, $ort, $projekt, $paketauswahl,$art,"",$abrechnen,$verrechnungsart,$kostenstelle,$gps); 
