...
//    $this->app->erp->MenuEintrag("index.php?module=shopimport&action=alle","Alle importieren");
  ...
  function ShopimportAlle()
  {
    ...
    $shops = $this->app->DB->SelectArr("SELECT ae.id as id FROM shopexport ae LEFT JOIN projekt p ON p.id=ae.projekt WHERE ae.aktiv='1'");
	  			...
	  			$this->app->DB->Update("UPDATE shopimport_auftraege SET trash='1' WHERE id='$shopimportid' LIMIT 1");
	    				...
	    				{ 
								...
								if(strlen($warenkorb[kundennummer])!="")
									$adresse = $this->app->DB->Select("SELECT id FROM adresse WHERE kundennummer='{$warenkorb[kundennummer]}' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1");
								if($adresse<=0) {
...
//exit;
				...
				$msg = $this->app->erp->base64_url_encode("<div class=\"error\">$sucess_import Auftr&auml;ge importiert!</div>");
    ...
    $htmltable->AddRowAsHeading(array('Import','M&uuml;ll','Sp&auml;ter','Internet','Name','Strasse','PLZ','Ort','Kd.Nr.','Kunde','Land','Betrag','Offen','Zahlung','Partner'));
    ...
    for($i=0;$i<count($arr);$i++)
    {
      ...
      $checkid = $this->app->DB->Select("SELECT kundennummer FROM adresse WHERE name='".$warenkorb[name]."' AND email='".$warenkorb[email]."' AND abteilung='".$warenkorb[abteilung]."'
        AND strasse='".$warenkorb[strasse]."' AND plz='".$warenkorb[plz]."' AND ort='".$warenkorb[ort]."' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1");
      ...
      $validkundennummer = $this->app->DB->Select("SELECT kundennummer FROM adresse WHERE kundennummer='".$warenkorb[kundennummer]."' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1");

      if(is_numeric($warenkorb[kundennummer]) && $validkundennummer==$warenkorb[kundennummer])
      {
				$kdrnummer = $warenkorb[kundennummer];
				$kdr_name = $this->app->DB->Select("SELECT name FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1");
        $email = $this->app->DB->Select("SELECT email FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1");
				$parts= explode("\n", wordwrap($kdr_name, 20, "\n"));
				$kdr_name = $parts[0];
				$kdr_name = "<br><i style=\"color:red\">".$kdr_name."</i>";
        $email = "<br><i style=\"color:red\">".$email."</i>";

				$kdr_strasse = $this->app->DB->Select("SELECT strasse FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1");
				$parts= explode("\n", wordwrap($kdr_strasse, 20, "\n"));
				$kdr_strasse = $parts[0];
				$kdr_strasse = "<br><i style=\"color:red\">".$kdr_strasse."</i>";

				$kdr_plz = "<br><i style=\"color:red\">".$this->app->DB->Select("SELECT plz FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1")."</i>";
				$kdr_ort = "<br><i style=\"color:red\">".$this->app->DB->Select("SELECT ort FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1")."</i>";
				$checked="";
      } 
			elseif (is_numeric($checkid))
      {
        ...
        $kdr_plz = "<br><i style=\"color:green\">".$this->app->DB->Select("SELECT plz FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1")."</i>";
        $kdr_ort = "<br><i style=\"color:green\">".$this->app->DB->Select("SELECT ort FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1")."</i>";

      } 
			elseif (is_numeric($checkidemail))
      {
        $checked="checked";
        ...
        $kdr_plz = "<br><i style=\"color:purple\">".$this->app->DB->Select("SELECT plz FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1")."</i>";
        $kdr_ort = "<br><i style=\"color:purple\">".$this->app->DB->Select("SELECT ort FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1")."</i>";
        ...
        $kdrnummer="";
			...
			if($kdrnummer != "")
      	$kdr_addresse_id  = $this->app->DB->Select("SELECT id FROM adresse WHERE kundennummer='$kdrnummer' AND firma='".$this->app->User->GetFirma()."' AND geloescht!=1 LIMIT 1");
