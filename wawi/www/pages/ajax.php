...
class Ajax {
  ...
  function AjaxTooltipSuche()
  {
	...
	function AjaxLieferadresse()
	{
  	...
  	echo $this->app->erp->ClearDataBeforeOutput($values[0][name]."#*#".$values[0][abteilung]."#*#".$values[0][unterabteilung]."#*#".$values[0][land]."#*#".$values[0][strasse]."#*#".$values[0][ort]."#*#".$values[0][plz]."#*#".$values[0][adresszusatz]."#*#".$values[0][ansprechpartner]);
		exit;
	...
	function AjaxAnsprechpartner()
	{
  	...
  	echo $this->app->erp->ClearDataBeforeOutput($values[0][name]."#*#".$values[0][email]."#*#".$values[0][telefon]."#*#".$values[0][telefax]."#*#".$values[0][abteilung]."#*#".$values[0][unterabteilung].
				"#*#".$values[0][ansprechpartner_land]."#*#".$values[0][strasse]."#*#".$values[0][plz]."#*#".$values[0][ort]."#*#".$values[0][adresszusatz]."#*#".$values[0][typ]."#*#".$values[0][anschreiben]);
		exit;
...
//		$term = str_replace(' ','%',$term);
	...
	case "artikelname":
	  $arr = $this->app->DB->SelectArr("SELECT name_de FROM artikel WHERE geloescht=0 AND intern_gesperrt!=1 AND firma='".$this->app->User->GetFirma()."' AND (name_de LIKE '%$term%' OR nummer LIKE '%$term%') AND geloescht=0 ORDER by name_de LIMIT 20");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name_de'];
	break;

	case "artikelgruppe":
	  $arr = $this->app->DB->SelectArr("SELECT DISTINCT typ FROM artikel WHERE geloescht=0 AND intern_gesperrt!=1 AND firma='".$this->app->User->GetFirma()."' AND typ LIKE '%$term%' ORDER by typ");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['typ'];
	break;
	...
	case "seriennummer":
	  ...
	  $arr = $this->app->DB->SelectArr("SELECT DISTINCT seriennummer FROM seriennummern WHERE seriennummer LIKE '%$term%' ORDER by seriennummer");
	  for($i=0;$i<count($arr);$i++)
	...
	case "hersteller":
	  $arr = $this->app->DB->SelectArr("SELECT DISTINCT hersteller FROM artikel WHERE geloescht=0 AND intern_gesperrt!=1 AND firma='".$this->app->User->GetFirma()."' AND hersteller LIKE '%$term%' ORDER by hersteller");
	  for($i=0;$i<count($arr);$i++)
	...
	case "bestellung":
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(belegnr,' ',name,' ',DATE_FORMAT(datum,'%d.%m.%Y')) as name 
	    FROM bestellung WHERE belegnr>0 AND firma='".$this->app->User->GetFirma()."' AND (name LIKE '%$term%' OR belegnr LIKE '%$term%' OR DATE_FORMAT(datum,'%Y-%m-%d') LIKE '%$term%')  ORDER by belegnr LIMIT 20");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name'];
	break;
	...
	case "arbeitsnachweis":
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(belegnr,' ',name,' ',DATE_FORMAT(datum,'%d.%m.%Y')) as name 
	    FROM arbeitsnachweis WHERE belegnr>0 AND firma='".$this->app->User->GetFirma()."' AND (name LIKE '%$term%' OR belegnr LIKE '%$term%' OR DATE_FORMAT(datum,'%Y-%m-%d') LIKE '%$ter    m%') ORDER by belegnr LIMIT 20");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name'];
	break;
	...
	case "herstellerlink":
	  $arr = $this->app->DB->SelectArr("SELECT DISTINCT herstellerlink FROM artikel WHERE geloescht=0 AND intern_gesperrt!=1 AND firma='".$this->app->User->GetFirma()."' AND herstellerlink LIKE '%$term%' ORDER by herstellerlink");
	  for($i=0;$i<count($arr);$i++)
	...
	case "sachkonto":
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(sachkonto,' ',beschriftung) as name FROM kontorahmen 
				WHERE (beschriftung LIKE '%$term%' OR sachkonto LIKE '%$term%' OR sachkonto LIKE '%$term2%' OR sachkonto LIKE '%$term3%') AND ausblenden!=1 ORDER by sachkonto");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name'];
	break;
	...
	case "artikelnummer":
	  ...
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(nummer,' ',name_de) as name FROM artikel WHERE geloescht=0 AND firma='".$this->app->User->GetFirma()."' AND (nummer LIKE '%$term%' OR name_de LIKE '%$term%') AND geloescht=0 AND intern_gesperrt!=1 $tmp_where LIMIT 20");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name'];
	break;
	...
	case "lagerartikelnummer":
		$arr = $this->app->DB->SelectArr("SELECT CONCAT(nummer,' ',name_de) as name 
					FROM artikel WHERE firma='".$this->app->User->GetFirma()."' AND (nummer LIKE '%$term%' OR name_de LIKE '%$term%') AND geloescht=0  AND intern_gesperrt!=1
	...
	case "lagerartikelnummerprojekt":
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(a.nummer,' ',a.name_de,' (',p.abkuerzung,')') as name FROM artikel a LEFT JOIN projekt p ON p.id=a.projekt WHERE a.geloescht=0 AND a.porto=0 AND a.intern_gesperrt!=1 AND a.firma='".$this->app->User->GetFirma()."' 
    ...
    $arr = $this->app->DB->SelectArr("SELECT a.id as id, CONCAT(a.nummer,' ',a.name_de,' (',p.abkuerzung,if(a.lagerartikel=1,'',''),')',if(a.herstellernummer!='',CONCAT(' (PN: ',a.herstellernummer,')'),'')) as name FROM artikel a LEFT JOIN projekt p ON p.id=a.projekt WHERE a.geloescht=0 AND a.intern_gesperrt!=1 AND a.firma='".$this->app->User->GetFirma()."' 
    AND (a.nummer LIKE '%$term%' OR a.name_de LIKE '%$term%' OR a.herstellernummer LIKE '%$term%' OR a.nummer LIKE '%$term2%' OR a.name_de LIKE '%$term2%' OR a.herstellernummer LIKE '%$term2%' OR a.nummer LIKE '%$term3%' OR a.name_de LIKE '%$term3%' OR a.herstellernummer LIKE '%$term3%') $tmp_where LIMIT 20");
	...
	case "einkaufartikelnummerprojekt":
	  ...
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(a.nummer,' ', a.name_de,' | Bezeichnung bei Lieferant ',e.bestellnummer,' ',LEFT(e.bezeichnunglieferant,50),' | ',' ab Menge ',e.ab_menge, ' | Preis ',e.preis) as name FROM artikel a 
	    LEFT JOIN projekt p ON p.id=a.projekt LEFT JOIN einkaufspreise e ON e.artikel=a.id WHERE a.geloescht=0 AND a.intern_gesperrt!=1 AND (e.gueltig_bis > NOW() OR e.gueltig_bis='0000-00-00')AND a.firma='".$this->app->User->GetFirma()."' AND e.adresse='$adresse' 
	...
	case "lieferantname":
	  $arr = $this->app->DB->SelectArr("SELECT name FROM adresse WHERE geloescht=0 AND firma='".$this->app->User->GetFirma()."' AND a.lieferantennummer!='' AND lieferantennummer!=0 AND (name LIKE '%$term%' OR name LIKE '%$term2%' OR name LIKE '%$term3%') order by name LIMIT 20");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name'];
	break;

	case "lieferant":
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(a.lieferantennummer,' ',a.name) as name FROM adresse a LEFT JOIN projekt p ON p.id=a.projekt WHERE a.geloescht=0 AND a.firma='".$this->app->User->GetFirma()."' AND a.lieferantennummer!='' AND lieferantennummer!=0 AND (a.name LIKE '%$term%' OR a.lieferantennummer LIKE '%$term%' OR a.name LIKE '%$term2%' OR a.name LIKE '%$term3%') ".$this->app->erp->ProjektRechte()." order by a.name LIMIT 20");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name'];
	break;
	...
	case "kunde":
		...
		{
	  	$arr = $this->app->DB->SelectArr("SELECT CONCAT(a.kundennummer,' ',a.name,if(a.freifeld1!='',CONCAT(' (',a.freifeld1,')'),'')) as name FROM adresse a LEFT JOIN projekt p ON p.id=a.projekt WHERE a.geloescht=0 AND a.firma='".$this->app->User->GetFirma()."' AND a.kundennummer!=0 AND (a.name LIKE '%$term%' OR a.kundennummer LIKE '%$term%' OR a.freifeld1 LIKE '%$term%' OR a.plz LIKE '%$term%' OR a.name LIKE '%$term2%' OR a.name LIKE '%$term3%') ".$this->app->erp->ProjektRechte()." order by name LIMIT 20");
		} else {
	  	$arr = $this->app->DB->SelectArr("SELECT CONCAT(a.kundennummer,' ',a.name) as name FROM adresse a LEFT JOIN projekt p ON p.id=a.projekt WHERE a.geloescht=0 AND a.firma='".$this->app->User->GetFirma()."' AND a.kundennummer!=0 AND (a.name LIKE '%$term%' OR a.kundennummer LIKE '%$term%' OR a.name LIKE '%$term2%' OR a.name LIKE '%$term3%') ".$this->app->erp->ProjektRechte()." order by name LIMIT 20");
		}
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name'];
	break;
	...
	case "gruppekennziffer":
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(kennziffer,' ',name) as bezeichnung FROM gruppen WHERE name LIKE '%$term%' OR kennziffer LIKE '%$term%'");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['bezeichnung'];
	break;

	case "gruppe":
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(name,' ',kennziffer) as bezeichnung FROM gruppen WHERE name LIKE '%$term%' OR kennziffer LIKE '%$term%'");
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['bezeichnung'];
	break;
	...
	case "projektname":
	  $arr = $this->app->DB->SelectArr("SELECT CONCAT(p.abkuerzung,' ',p.name) as name FROM projekt p WHERE p.geloescht=0 AND p.firma='".$this->app->User->GetFirma()."' AND (p.name LIKE '%$term%' OR p.abkuerzung LIKE '%$term%') ".$this->app->erp->ProjektRechte());
	  for($i=0;$i<count($arr);$i++)
	    $newarr[] = $arr[$i]['name'];
	break;
...
exit;
  ...
  if ( isset($iDisplayStart) )
  {
    ...
    $sLimit = "LIMIT ".mysqli_real_escape_string($this->app->DB->connection, $iDisplayLength )." OFFSET ".
      mysqli_real_escape_string($this->app->DB->connection, $iDisplayStart );
		}else{
		$sLimit = "LIMIT ".mysqli_real_escape_string($this->app->DB->connection, $iDisplayStart ).", ".
      mysqli_real_escape_string($this->app->DB->connection, $iDisplayLength );
		}
  ...
  $moreinfo = $this->app->YUI->TableSearch("",$cmd,"moreinfo");
...
*/
    ...
    $sWhere .=" OR ({$searchsql[$i]} LIKE '%".mysqli_real_escape_string($this->app->DB->connection, $sSearch2 )."%' OR {$searchsql[$i]} LIKE '%".$this->app->erp->ConvertForTableSearch( $sSearch2 )."%' ) ";
      ...
      if($sSearch3!="")
        $sWhere .= "OR ({$searchsql[$i]} LIKE '%".mysqli_real_escape_string($this->app->DB->connection, $sSearch3 )."%' OR {$searchsql[$i]} LIKE '%".$this->app->erp->ConvertForTableSearch( $sSearch3 )."%' ) OR ";
  			...
  			$searchfulltext = $this->app->YUI->TableSearch("",$cmd,"searchfulltext");
				if($searchfulltext!="" && $sSearch!="")
				{
					$sSearch = str_replace("&quot;",'"',$sSearch);
					$sSearch .= "*";
  ...
  $groupby = $this->app->YUI->TableSearch("",$cmd,"groupby");
