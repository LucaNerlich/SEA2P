...
class Kalender {
  ...
  //		$this->app->Tpl->Parse(TAB1,"kalender.tpl");
	...
	{
		$findlogin = $this->app->DB->Select("SELECT id FROM user WHERE username='".$_SERVER['PHP_AUTH_USER']."' AND username!=''
			AND kalender_aktiv='1' AND kalender_passwort='".$_SERVER['PHP_AUTH_PW']."' AND kalender_passwort!='' AND `activ`='1' LIMIT 1");

    $this->app->erp->Protokoll("Benutzer: ".$_SERVER['PHP_AUTH_USER']);
