...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO lager_bewegung (id,lager_platz,artikel,menge,vpe,eingang,zeit,referenz,bearbeiter,projekt,firma,logdatei,adresse)
      VALUES(DEFAULT, ".((is_numeric($this->lager_platz)) ? $this->lager_platz : '0').", ".((is_numeric($this->artikel)) ? $this->artikel : '0').", ".((is_numeric($this->menge)) ? $this->menge : '0').", '{$this->vpe}', ".((is_numeric($this->eingang)) ? $this->eingang : '0').", ".(($this->zeit=='' || $this->zeit=='--') ? 'NOW()' : "'".$this->zeit."'").", '{$this->referenz}', '{$this->bearbeiter}', ".((is_numeric($this->projekt)) ? $this->projekt : '0').", ".((is_numeric($this->firma)) ? $this->firma : '0').", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->adresse)) ? $this->adresse : '0').")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE lager_bewegung SET
      ...
      vpe='{$this->vpe}',
      eingang=".((is_numeric($this->eingang)) ? $this->eingang : '0').",
