...
class ObjGenUnterprojekt
{
  ...
  public function ObjGenUnterprojekt($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO unterprojekt (id,projekt,name,verantwortlicher,aktiv,position,logdatei)
      VALUES(DEFAULT, ".((is_numeric($this->projekt)) ? $this->projekt : '0').", '{$this->name}', '{$this->verantwortlicher}', '{$this->aktiv}', ".((is_numeric($this->position)) ? $this->position : '0').", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").")"; 
