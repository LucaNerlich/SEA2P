...
class ObjGenJqcalendar
{
  ...
  public function ObjGenJqcalendar($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO jqcalendar (id,adresse,titel,beschreibung,ort,von,bis,public)
      VALUES(DEFAULT, ".((is_numeric($this->adresse)) ? $this->adresse : '0').", '{$this->titel}', '{$this->beschreibung}', '{$this->ort}', ".(($this->von=='' || $this->von=='--') ? 'NOW()' : "'".$this->von."'").", ".(($this->bis=='' || $this->bis=='--') ? 'NOW()' : "'".$this->bis."'").", ".((is_numeric($this->public)) ? $this->public : '0').")"; 
