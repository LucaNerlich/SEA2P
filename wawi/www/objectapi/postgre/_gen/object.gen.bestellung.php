...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO bestellung (id,datum,projekt,bestellungsart,belegnr,bearbeiter,angebot,freitext,internebemerkung,status,adresse,name,vorname,abteilung,unterabteilung,strasse,adresszusatz,plz,ort,land,abweichendelieferadresse,liefername,lieferabteilung,lieferunterabteilung,lieferland,lieferstrasse,lieferort,lieferplz,lieferadresszusatz,lieferansprechpartner,ustid,ust_befreit,email,telefon,telefax,betreff,kundennummer,lieferantennummer,versandart,lieferdatum,einkaeufer,keineartikelnummern,zahlungsweise,zahlungsstatus,zahlungszieltage,zahlungszieltageskonto,zahlungszielskonto,gesamtsumme,bank_inhaber,bank_institut,bank_blz,bank_konto,paypalaccount,bestellbestaetigung,firma,versendet,versendet_am,versendet_per,versendet_durch,logdatei,artikelnummerninfotext,ansprechpartner,ohne_briefpapier,pdfarchiviert,pdfarchiviertversion,schreibschutz)
      VALUES(DEFAULT, ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", '{$this->projekt}', '{$this->bestellungsart}', '{$this->belegnr}', '{$this->bearbeiter}', '{$this->angebot}', '{$this->freitext}', '{$this->internebemerkung}', '{$this->status}', ".((is_numeric($this->adresse)) ? $this->adresse : '0').", '{$this->name}', '{$this->vorname}', '{$this->abteilung}', '{$this->unterabteilung}', '{$this->strasse}', '{$this->adresszusatz}', '{$this->plz}', '{$this->ort}', '{$this->land}', ".((is_numeric($this->abweichendelieferadresse)) ? $this->abweichendelieferadresse : '0').", '{$this->liefername}', '{$this->lieferabteilung}', '{$this->lieferunterabteilung}', '{$this->lieferland}', '{$this->lieferstrasse}', '{$this->lieferort}', '{$this->lieferplz}', '{$this->lieferadresszusatz}', '{$this->lieferansprechpartner}', '{$this->ustid}', ".((is_numeric($this->ust_befreit)) ? $this->ust_befreit : '0').", '{$this->email}', '{$this->telefon}', '{$this->telefax}', '{$this->betreff}', '{$this->kundennummer}', '{$this->lieferantennummer}', '{$this->versandart}', ".(($this->lieferdatum=='' || $this->lieferdatum=='--') ? 'NOW()' : "'".$this->lieferdatum."'").", '{$this->einkaeufer}', ".((is_numeric($this->keineartikelnummern)) ? $this->keineartikelnummern : '0').", '{$this->zahlungsweise}', '{$this->zahlungsstatus}', ".((is_numeric($this->zahlungszieltage)) ? $this->zahlungszieltage : '0').", ".((is_numeric($this->zahlungszieltageskonto)) ? $this->zahlungszieltageskonto : '0').", ".((is_numeric($this->zahlungszielskonto)) ? $this->zahlungszielskonto : '0').", ".((is_numeric($this->gesamtsumme)) ? $this->gesamtsumme : '0').", '{$this->bank_inhaber}', '{$this->bank_institut}', ".((is_numeric($this->bank_blz)) ? $this->bank_blz : '0').", ".((is_numeric($this->bank_konto)) ? $this->bank_konto : '0').", '{$this->paypalaccount}', ".((is_numeric($this->bestellbestaetigung)) ? $this->bestellbestaetigung : '0').", ".((is_numeric($this->firma)) ? $this->firma : '0').", ".((is_numeric($this->versendet)) ? $this->versendet : '0').", ".(($this->versendet_am=='' || $this->versendet_am=='--') ? 'NOW()' : "'".$this->versendet_am."'").", '{$this->versendet_per}', '{$this->versendet_durch}', ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->artikelnummerninfotext)) ? $this->artikelnummerninfotext : '0').", '{$this->ansprechpartner}', ".((is_numeric($this->ohne_briefpapier)) ? $this->ohne_briefpapier : '0').", ".((is_numeric($this->pdfarchiviert)) ? $this->pdfarchiviert : '0').", ".((is_numeric($this->pdfarchiviertversion)) ? $this->pdfarchiviertversion : '0').", ".((is_numeric($this->schreibschutz)) ? $this->schreibschutz : '0').")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE bestellung SET
      ...
      keineartikelnummern=".((is_numeric($this->keineartikelnummern)) ? $this->keineartikelnummern : '0').",
      ...
      bestellbestaetigung=".((is_numeric($this->bestellbestaetigung)) ? $this->bestellbestaetigung : '0').",
      ...
      artikelnummerninfotext=".((is_numeric($this->artikelnummerninfotext)) ? $this->artikelnummerninfotext : '0').",
