...
class ObjGenShopexport_Kampange
{
  ...
  public function ObjGenShopexport_Kampange($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO shopexport_kampange (id,name,banner,unterbanner,von,bis,link,firma,views,clicks,aktiv,shop,artikel,aktion,geloescht)
      VALUES(DEFAULT, '{$this->name}', ".((is_numeric($this->banner)) ? $this->banner : '0').", ".((is_numeric($this->unterbanner)) ? $this->unterbanner : '0').", ".(($this->von=='' || $this->von=='--') ? 'NOW()' : "'".$this->von."'").", ".(($this->bis=='' || $this->bis=='--') ? 'NOW()' : "'".$this->bis."'").", '{$this->link}', ".((is_numeric($this->firma)) ? $this->firma : '0').", ".((is_numeric($this->views)) ? $this->views : '0').", ".((is_numeric($this->clicks)) ? $this->clicks : '0').", ".((is_numeric($this->aktiv)) ? $this->aktiv : '0').", ".((is_numeric($this->shop)) ? $this->shop : '0').", '{$this->artikel}', '{$this->aktion}', ".((is_numeric($this->geloescht)) ? $this->geloescht : '0').")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE shopexport_kampange SET
      name='{$this->name}',
      banner=".((is_numeric($this->banner)) ? $this->banner : '0').",
      unterbanner=".((is_numeric($this->unterbanner)) ? $this->unterbanner : '0').",
      von=".(($this->von=='' || $this->von=='--') ? 'NOW()' : "'".$this->von."'").",
      bis=".(($this->bis=='' || $this->bis=='--') ? 'NOW()' : "'".$this->bis."'").",
      link='{$this->link}',
      firma=".((is_numeric($this->firma)) ? $this->firma : '0').",
      views=".((is_numeric($this->views)) ? $this->views : '0').",
      clicks=".((is_numeric($this->clicks)) ? $this->clicks : '0').",
