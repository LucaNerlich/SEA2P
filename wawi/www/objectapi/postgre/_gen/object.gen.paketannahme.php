...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO paketannahme (id,adresse,datum,verpackungszustand,bemerkung,foto,gewicht,bearbeiter,projekt,vorlage,vorlageid,zahlung,betrag,status,beipack_rechnung,beipack_lieferschein,beipack_anschreiben,beipack_gesamt,bearbeiter_distribution,postgrund,logdatei)
      VALUES(DEFAULT, ".((is_numeric($this->adresse)) ? $this->adresse : '0').", ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", ".((is_numeric($this->verpackungszustand)) ? $this->verpackungszustand : '0').", '{$this->bemerkung}', ".((is_numeric($this->foto)) ? $this->foto : '0').", '{$this->gewicht}', '{$this->bearbeiter}', ".((is_numeric($this->projekt)) ? $this->projekt : '0').", '{$this->vorlage}', '{$this->vorlageid}', '{$this->zahlung}', ".((is_numeric($this->betrag)) ? $this->betrag : '0').", '{$this->status}', ".((is_numeric($this->beipack_rechnung)) ? $this->beipack_rechnung : '0').", ".((is_numeric($this->beipack_lieferschein)) ? $this->beipack_lieferschein : '0').", ".((is_numeric($this->beipack_anschreiben)) ? $this->beipack_anschreiben : '0').", ".((is_numeric($this->beipack_gesamt)) ? $this->beipack_gesamt : '0').", '{$this->bearbeiter_distribution}', '{$this->postgrund}', ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE paketannahme SET
      ...
      verpackungszustand=".((is_numeric($this->verpackungszustand)) ? $this->verpackungszustand : '0').",
      bemerkung='{$this->bemerkung}',
      foto=".((is_numeric($this->foto)) ? $this->foto : '0').",
      ...
      status='{$this->status}',
      beipack_rechnung=".((is_numeric($this->beipack_rechnung)) ? $this->beipack_rechnung : '0').",
      beipack_lieferschein=".((is_numeric($this->beipack_lieferschein)) ? $this->beipack_lieferschein : '0').",
      beipack_anschreiben=".((is_numeric($this->beipack_anschreiben)) ? $this->beipack_anschreiben : '0').",
      beipack_gesamt=".((is_numeric($this->beipack_gesamt)) ? $this->beipack_gesamt : '0').",
