...
class ObjGenKontoauszuege
{
  ...
  public function ObjGenKontoauszuege($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO kontoauszuege (id,konto,buchung,vorgang,soll,haben,gebuehr,waehrung,fertig,datev_abgeschlossen,buchungstext,gegenkonto,belegfeld1,bearbeiter,mailbenachrichtigung,pruefsumme,internebemerkung,importfehler,importgroup)
      VALUES(DEFAULT, ".((is_numeric($this->konto)) ? $this->konto : '0').", ".(($this->buchung=='' || $this->buchung=='--') ? 'NOW()' : "'".$this->buchung."'").", '{$this->vorgang}', ".((is_numeric($this->soll)) ? $this->soll : '0').", ".((is_numeric($this->haben)) ? $this->haben : '0').", ".((is_numeric($this->gebuehr)) ? $this->gebuehr : '0').", '{$this->waehrung}', ".((is_numeric($this->fertig)) ? $this->fertig : '0').", ".((is_numeric($this->datev_abgeschlossen)) ? $this->datev_abgeschlossen : '0').", '{$this->buchungstext}', '{$this->gegenkonto}', '{$this->belegfeld1}', '{$this->bearbeiter}', ".((is_numeric($this->mailbenachrichtigung)) ? $this->mailbenachrichtigung : '0').", '{$this->pruefsumme}', '{$this->internebemerkung}', ".((is_numeric($this->importfehler)) ? $this->importfehler : '0').", ".((is_numeric($this->importgroup)) ? $this->importgroup : '0').")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE kontoauszuege SET
      konto=".((is_numeric($this->konto)) ? $this->konto : '0').",
      buchung=".(($this->buchung=='' || $this->buchung=='--') ? 'NOW()' : "'".$this->buchung."'").",
      vorgang='{$this->vorgang}',
      soll=".((is_numeric($this->soll)) ? $this->soll : '0').",
      haben=".((is_numeric($this->haben)) ? $this->haben : '0').",
      gebuehr=".((is_numeric($this->gebuehr)) ? $this->gebuehr : '0').",
      waehrung='{$this->waehrung}',
      fertig=".((is_numeric($this->fertig)) ? $this->fertig : '0').",
      ...
      mailbenachrichtigung=".((is_numeric($this->mailbenachrichtigung)) ? $this->mailbenachrichtigung : '0').",
      pruefsumme='{$this->pruefsumme}',
      internebemerkung='{$this->internebemerkung}',
      importfehler=".((is_numeric($this->importfehler)) ? $this->importfehler : '0').",
      importgroup=".((is_numeric($this->importgroup)) ? $this->importgroup : '0')."
      WHERE (id='{$this->id}')";
