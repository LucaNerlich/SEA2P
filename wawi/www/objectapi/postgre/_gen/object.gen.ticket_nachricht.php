...
class ObjGenTicket_Nachricht
{
  ...
  public function ObjGenTicket_Nachricht($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO ticket_nachricht (id,ticket,verfasser,bearbeiter,mail,zeit,zeitausgang,text,textausgang,betreff,bemerkung,medium,versendet,status)
      VALUES(DEFAULT, '{$this->ticket}', '{$this->verfasser}', '{$this->bearbeiter}', '{$this->mail}', ".(($this->zeit=='' || $this->zeit=='--') ? 'NOW()' : "'".$this->zeit."'").", ".(($this->zeitausgang=='' || $this->zeitausgang=='--') ? 'NOW()' : "'".$this->zeitausgang."'").", '{$this->text}', '{$this->textausgang}', '{$this->betreff}', '{$this->bemerkung}', '{$this->medium}', '{$this->versendet}', '{$this->status}')"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE ticket_nachricht SET
      ...
      zeitausgang=".(($this->zeitausgang=='' || $this->zeitausgang=='--') ? 'NOW()' : "'".$this->zeitausgang."'").",
      text='{$this->text}',
