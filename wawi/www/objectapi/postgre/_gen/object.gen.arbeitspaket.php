...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO arbeitspaket (id,adresse,aufgabe,beschreibung,projekt,zeit_geplant,kostenstelle,status,abgabe,abgenommen,abgenommen_von,abgenommen_bemerkung,initiator,art,abgabedatum,logdatei,geloescht,auftragid,vorgaenger,kosten_geplant,artikel_geplant)
      VALUES(DEFAULT, ".((is_numeric($this->adresse)) ? $this->adresse : '0').", '{$this->aufgabe}', '{$this->beschreibung}', ".((is_numeric($this->projekt)) ? $this->projekt : '0').", ".((is_numeric($this->zeit_geplant)) ? $this->zeit_geplant : '0').", ".((is_numeric($this->kostenstelle)) ? $this->kostenstelle : '0').", '{$this->status}', '{$this->abgabe}', '{$this->abgenommen}', ".((is_numeric($this->abgenommen_von)) ? $this->abgenommen_von : '0').", '{$this->abgenommen_bemerkung}', ".((is_numeric($this->initiator)) ? $this->initiator : '0').", '{$this->art}', '{$this->abgabedatum}', ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->geloescht)) ? $this->geloescht : '0').", ".((is_numeric($this->auftragid)) ? $this->auftragid : '0').", ".((is_numeric($this->vorgaenger)) ? $this->vorgaenger : '0').", ".((is_numeric($this->kosten_geplant)) ? $this->kosten_geplant : '0').", ".((is_numeric($this->artikel_geplant)) ? $this->artikel_geplant : '0').")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE arbeitspaket SET
      ...
      zeit_geplant=".((is_numeric($this->zeit_geplant)) ? $this->zeit_geplant : '0').",
      kostenstelle=".((is_numeric($this->kostenstelle)) ? $this->kostenstelle : '0').",
      status='{$this->status}',
      abgabe='{$this->abgabe}',
      abgenommen='{$this->abgenommen}',
      abgenommen_von=".((is_numeric($this->abgenommen_von)) ? $this->abgenommen_von : '0').",
      ...
      vorgaenger=".((is_numeric($this->vorgaenger)) ? $this->vorgaenger : '0').",
      kosten_geplant=".((is_numeric($this->kosten_geplant)) ? $this->kosten_geplant : '0').",
      artikel_geplant=".((is_numeric($this->artikel_geplant)) ? $this->artikel_geplant : '0')."
      WHERE (id='{$this->id}')";
