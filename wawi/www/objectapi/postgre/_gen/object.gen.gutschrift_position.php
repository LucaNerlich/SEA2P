...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO gutschrift_position (id,gutschrift,artikel,projekt,bezeichnung,beschreibung,internerkommentar,nummer,menge,preis,waehrung,lieferdatum,vpe,sort,status,umsatzsteuer,bemerkung,logdatei,rabatt)
      VALUES(DEFAULT, ".((is_numeric($this->gutschrift)) ? $this->gutschrift : '0').", '{$this->artikel}', ".((is_numeric($this->projekt)) ? $this->projekt : '0').", '{$this->bezeichnung}', '{$this->beschreibung}', '{$this->internerkommentar}', '{$this->nummer}', '{$this->menge}', ".((is_numeric($this->preis)) ? $this->preis : '0').", '{$this->waehrung}', ".(($this->lieferdatum=='' || $this->lieferdatum=='--') ? 'NOW()' : "'".$this->lieferdatum."'").", '{$this->vpe}', ".((is_numeric($this->sort)) ? $this->sort : '0').", '{$this->status}', '{$this->umsatzsteuer}', '{$this->bemerkung}', ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->rabatt)) ? $this->rabatt : '0').")"; 
