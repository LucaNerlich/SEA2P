...
class ObjGenUstprf_Protokoll
{
  ...
  public function ObjGenUstprf_Protokoll($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO ustprf_protokoll (id,ustprf_id,zeit,bemerkung,bearbeiter,logdatei)
      VALUES(DEFAULT, ".((is_numeric($this->ustprf_id)) ? $this->ustprf_id : '0').", ".(($this->zeit=='' || $this->zeit=='--') ? 'NOW()' : "'".$this->zeit."'").", '{$this->bemerkung}', '{$this->bearbeiter}', ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE ustprf_protokoll SET
      ustprf_id=".((is_numeric($this->ustprf_id)) ? $this->ustprf_id : '0').",
