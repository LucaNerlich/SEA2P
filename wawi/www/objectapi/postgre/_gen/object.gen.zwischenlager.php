...
class ObjGenZwischenlager
{
  ...
  public function ObjGenZwischenlager($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO zwischenlager (id,bearbeiter,projekt,artikel,menge,vpe,grund,lager_von,lager_nach,richtung,erledigt,objekt,parameter,firma,logdatei)
      VALUES(DEFAULT, '{$this->bearbeiter}', ".((is_numeric($this->projekt)) ? $this->projekt : '0').", ".((is_numeric($this->artikel)) ? $this->artikel : '0').", ".((is_numeric($this->menge)) ? $this->menge : '0').", '{$this->vpe}', '{$this->grund}', '{$this->lager_von}', '{$this->lager_nach}', '{$this->richtung}', ".((is_numeric($this->erledigt)) ? $this->erledigt : '0').", '{$this->objekt}', '{$this->parameter}', ".((is_numeric($this->firma)) ? $this->firma : '0').", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE zwischenlager SET
      ...
      erledigt=".((is_numeric($this->erledigt)) ? $this->erledigt : '0').",
      objekt='{$this->objekt}',
