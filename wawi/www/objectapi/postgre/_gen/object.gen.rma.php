...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO rma (id,datum,projekt,belegnr,bearbeiter,freigabe,status,adresse,name,vorname,abteilung,unterabteilung,strasse,adresszusatz,plz,ort,ustid,email,telefon,telefax,betreff,kundennummer,lieferantennummer,zahlungsweise,zahlungszieltage,versandart,bestellbestaetigung,freitext,zahlungszielskonto,zahlungszieltageskonto,bestellbestaetigungsdatum,lieferdatum,einkaeufer,firma,logdatei)
      VALUES(DEFAULT, ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", '{$this->projekt}', ".((is_numeric($this->belegnr)) ? $this->belegnr : '0').", '{$this->bearbeiter}', ".((is_numeric($this->freigabe)) ? $this->freigabe : '0').", '{$this->status}', ".((is_numeric($this->adresse)) ? $this->adresse : '0').", '{$this->name}', '{$this->vorname}', '{$this->abteilung}', '{$this->unterabteilung}', '{$this->strasse}', '{$this->adresszusatz}', '{$this->plz}', '{$this->ort}', '{$this->ustid}', '{$this->email}', '{$this->telefon}', '{$this->telefax}', '{$this->betreff}', '{$this->kundennummer}', '{$this->lieferantennummer}', '{$this->zahlungsweise}', ".((is_numeric($this->zahlungszieltage)) ? $this->zahlungszieltage : '0').", '{$this->versandart}', ".((is_numeric($this->bestellbestaetigung)) ? $this->bestellbestaetigung : '0').", '{$this->freitext}', ".((is_numeric($this->zahlungszielskonto)) ? $this->zahlungszielskonto : '0').", ".((is_numeric($this->zahlungszieltageskonto)) ? $this->zahlungszieltageskonto : '0').", ".(($this->bestellbestaetigungsdatum=='' || $this->bestellbestaetigungsdatum=='--') ? 'NOW()' : "'".$this->bestellbestaetigungsdatum."'").", ".(($this->lieferdatum=='' || $this->lieferdatum=='--') ? 'NOW()' : "'".$this->lieferdatum."'").", '{$this->einkaeufer}', ".((is_numeric($this->firma)) ? $this->firma : '0').", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE rma SET
      ...
      belegnr=".((is_numeric($this->belegnr)) ? $this->belegnr : '0').",
      ...
      bestellbestaetigungsdatum=".(($this->bestellbestaetigungsdatum=='' || $this->bestellbestaetigungsdatum=='--') ? 'NOW()' : "'".$this->bestellbestaetigungsdatum."'").",
