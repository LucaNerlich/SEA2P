...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO bestellung_position (id,bestellung,artikel,projekt,bezeichnunglieferant,bestellnummer,beschreibung,menge,preis,waehrung,lieferdatum,vpe,sort,status,umsatzsteuer,bemerkung,geliefert,mengemanuellgeliefertaktiviert,manuellgeliefertbearbeiter,abgerechnet,logdatei,abgeschlossen)
      VALUES(DEFAULT, '{$this->bestellung}', '{$this->artikel}', ".((is_numeric($this->projekt)) ? $this->projekt : '0').", '{$this->bezeichnunglieferant}', '{$this->bestellnummer}', '{$this->beschreibung}', '{$this->menge}', ".((is_numeric($this->preis)) ? $this->preis : '0').", '{$this->waehrung}', ".(($this->lieferdatum=='' || $this->lieferdatum=='--') ? 'NOW()' : "'".$this->lieferdatum."'").", '{$this->vpe}', ".((is_numeric($this->sort)) ? $this->sort : '0').", '{$this->status}', '{$this->umsatzsteuer}', '{$this->bemerkung}', ".((is_numeric($this->geliefert)) ? $this->geliefert : '0').", ".((is_numeric($this->mengemanuellgeliefertaktiviert)) ? $this->mengemanuellgeliefertaktiviert : '0').", '{$this->manuellgeliefertbearbeiter}', ".((is_numeric($this->abgerechnet)) ? $this->abgerechnet : '0').", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->abgeschlossen)) ? $this->abgeschlossen : '0').")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE bestellung_position SET
      ...
      mengemanuellgeliefertaktiviert=".((is_numeric($this->mengemanuellgeliefertaktiviert)) ? $this->mengemanuellgeliefertaktiviert : '0').",
