...
class ObjGenDokumente
{
  ...
  public function ObjGenDokumente($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO dokumente (id,adresse_from,adresse_to,typ,von,firma,ansprechpartner,an,email_an,firma_an,adresse,plz,ort,land,datum,betreff,content,signatur,send_as,email,printer,fax,sent,deleted,created)
      VALUES(DEFAULT, ".((is_numeric($this->adresse_from)) ? $this->adresse_from : '0').", ".((is_numeric($this->adresse_to)) ? $this->adresse_to : '0').", '{$this->typ}', '{$this->von}', '{$this->firma}', '{$this->ansprechpartner}', '{$this->an}', '{$this->email_an}', '{$this->firma_an}', '{$this->adresse}', '{$this->plz}', '{$this->ort}', '{$this->land}', ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", '{$this->betreff}', '{$this->content}', ".((is_numeric($this->signatur)) ? $this->signatur : '0').", '{$this->send_as}', '{$this->email}', ".((is_numeric($this->printer)) ? $this->printer : '0').", ".((is_numeric($this->fax)) ? $this->fax : '0').", ".((is_numeric($this->sent)) ? $this->sent : '0').", ".((is_numeric($this->deleted)) ? $this->deleted : '0').", ".(($this->created=='' || $this->created=='--') ? 'NOW()' : "'".$this->created."'").")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE dokumente SET
      adresse_from=".((is_numeric($this->adresse_from)) ? $this->adresse_from : '0').",
      adresse_to=".((is_numeric($this->adresse_to)) ? $this->adresse_to : '0').",
      typ='{$this->typ}',
      von='{$this->von}',
      firma='{$this->firma}',
      ...
      signatur=".((is_numeric($this->signatur)) ? $this->signatur : '0').",
      send_as='{$this->send_as}',
      email='{$this->email}',
      printer=".((is_numeric($this->printer)) ? $this->printer : '0').",
      fax=".((is_numeric($this->fax)) ? $this->fax : '0').",
      sent=".((is_numeric($this->sent)) ? $this->sent : '0').",
      deleted=".((is_numeric($this->deleted)) ? $this->deleted : '0').",
      created=".(($this->created=='' || $this->created=='--') ? 'NOW()' : "'".$this->created."'")."
      WHERE (id='{$this->id}')";
