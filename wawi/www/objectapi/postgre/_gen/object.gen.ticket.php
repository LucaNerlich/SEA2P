...
class ObjGenTicket
{
  ...
  public function ObjGenTicket($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO ticket (id,schluessel,zeit,projekt,bearbeiter,quelle,status,adresse,kunde,warteschlange,mailadresse,prio,betreff,zugewiesen,inbearbeitung,inbearbeitung_user,firma,notiz)
      VALUES(DEFAULT, '{$this->schluessel}', ".(($this->zeit=='' || $this->zeit=='--') ? 'NOW()' : "'".$this->zeit."'").", '{$this->projekt}', '{$this->bearbeiter}', '{$this->quelle}', '{$this->status}', ".((is_numeric($this->adresse)) ? $this->adresse : '0').", '{$this->kunde}', '{$this->warteschlange}', '{$this->mailadresse}', ".((is_numeric($this->prio)) ? $this->prio : '0').", '{$this->betreff}', ".((is_numeric($this->zugewiesen)) ? $this->zugewiesen : '0').", ".((is_numeric($this->inbearbeitung)) ? $this->inbearbeitung : '0').", '{$this->inbearbeitung_user}', ".((is_numeric($this->firma)) ? $this->firma : '0').", '{$this->notiz}')"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE ticket SET
      ...
      prio=".((is_numeric($this->prio)) ? $this->prio : '0').",
      betreff='{$this->betreff}',
      zugewiesen=".((is_numeric($this->zugewiesen)) ? $this->zugewiesen : '0').",
