...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO ustprf (id,adresse,name,ustid,land,ort,plz,rechtsform,strasse,status,datum_online,datum_brief,bearbeiter,briefbestellt,logdatei)
      VALUES(DEFAULT, '{$this->adresse}', '{$this->name}', '{$this->ustid}', '{$this->land}', '{$this->ort}', '{$this->plz}', '{$this->rechtsform}', '{$this->strasse}', '{$this->status}', ".(($this->datum_online=='' || $this->datum_online=='--') ? 'NOW()' : "'".$this->datum_online."'").", ".(($this->datum_brief=='' || $this->datum_brief=='--') ? 'NOW()' : "'".$this->datum_brief."'").", '{$this->bearbeiter}', ".(($this->briefbestellt=='' || $this->briefbestellt=='--') ? 'NOW()' : "'".$this->briefbestellt."'").", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE ustprf SET
      ...
      status='{$this->status}',
      datum_online=".(($this->datum_online=='' || $this->datum_online=='--') ? 'NOW()' : "'".$this->datum_online."'").",
      datum_brief=".(($this->datum_brief=='' || $this->datum_brief=='--') ? 'NOW()' : "'".$this->datum_brief."'").",
      bearbeiter='{$this->bearbeiter}',
      briefbestellt=".(($this->briefbestellt=='' || $this->briefbestellt=='--') ? 'NOW()' : "'".$this->briefbestellt."'").",
