...
class ObjGenInhalt
{
  ...
  public function ObjGenInhalt($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO inhalt (id,sprache,inhalt,kurztext,html,title,description,keywords,inhaltstyp,sichtbarbis,datum,aktiv,shop,template,finalparse,navigation)
      VALUES(DEFAULT, '{$this->sprache}', '{$this->inhalt}', '{$this->kurztext}', '{$this->html}', '{$this->title}', '{$this->description}', '{$this->keywords}', '{$this->inhaltstyp}', ".(($this->sichtbarbis=='' || $this->sichtbarbis=='--') ? 'NOW()' : "'".$this->sichtbarbis."'").", ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", ".((is_numeric($this->aktiv)) ? $this->aktiv : '0').", ".((is_numeric($this->shop)) ? $this->shop : '0').", '{$this->template}', '{$this->finalparse}', '{$this->navigation}')"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE inhalt SET
      ...
      sichtbarbis=".(($this->sichtbarbis=='' || $this->sichtbarbis=='--') ? 'NOW()' : "'".$this->sichtbarbis."'").",
