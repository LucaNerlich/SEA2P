...
class ObjGenReisekosten_Position
{
  ...
  public function ObjGenReisekosten_Position($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO reisekosten_position (id,reisekosten,reisekostenart,artikel,projekt,bezeichnung,beschreibung,ort,internerkommentar,nummer,verrechnungsart,menge,arbeitspaket,datum,von,bis,sort,status,bemerkung,bezahlt_wie,uststeuersatz,keineust,betrag,abrechnen,abgerechnet,abgerechnet_objekt,abgerechnet_parameter,exportiert,exportiert_am,logdatei,mitarbeiter)
      VALUES(DEFAULT, ".((is_numeric($this->reisekosten)) ? $this->reisekosten : '0').", '{$this->reisekostenart}', '{$this->artikel}', ".((is_numeric($this->projekt)) ? $this->projekt : '0').", '{$this->bezeichnung}', '{$this->beschreibung}', '{$this->ort}', '{$this->internerkommentar}', '{$this->nummer}', '{$this->verrechnungsart}', '{$this->menge}', ".((is_numeric($this->arbeitspaket)) ? $this->arbeitspaket : '0').", ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", '{$this->von}', '{$this->bis}', ".((is_numeric($this->sort)) ? $this->sort : '0').", '{$this->status}', '{$this->bemerkung}', '{$this->bezahlt_wie}', '{$this->uststeuersatz}', ".((is_numeric($this->keineust)) ? $this->keineust : '0').", ".((is_numeric($this->betrag)) ? $this->betrag : '0').", ".((is_numeric($this->abrechnen)) ? $this->abrechnen : '0').", ".((is_numeric($this->abgerechnet)) ? $this->abgerechnet : '0').", '{$this->abgerechnet_objekt}', ".((is_numeric($this->abgerechnet_parameter)) ? $this->abgerechnet_parameter : '0').", ".((is_numeric($this->exportiert)) ? $this->exportiert : '0').", ".(($this->exportiert_am=='' || $this->exportiert_am=='--') ? 'NOW()' : "'".$this->exportiert_am."'").", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->mitarbeiter)) ? $this->mitarbeiter : '0').")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE reisekosten_position SET
      reisekosten=".((is_numeric($this->reisekosten)) ? $this->reisekosten : '0').",
      ...
      keineust=".((is_numeric($this->keineust)) ? $this->keineust : '0').",
      ...
      abgerechnet_parameter=".((is_numeric($this->abgerechnet_parameter)) ? $this->abgerechnet_parameter : '0').",
      exportiert=".((is_numeric($this->exportiert)) ? $this->exportiert : '0').",
      exportiert_am=".(($this->exportiert_am=='' || $this->exportiert_am=='--') ? 'NOW()' : "'".$this->exportiert_am."'").",
