...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO inventur (id,datum,projekt,belegnr,bearbeiter,auftrag,auftragid,freitext,status,adresse,mitarbeiter,name,abteilung,unterabteilung,strasse,adresszusatz,ansprechpartner,plz,ort,land,ustid,email,telefon,telefax,betreff,kundennummer,versandart,versand,firma,versendet,versendet_am,versendet_per,versendet_durch,inbearbeitung_user,logdatei,ohne_briefpapier,schreibschutz)
      VALUES(DEFAULT, ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", '{$this->projekt}', '{$this->belegnr}', '{$this->bearbeiter}', '{$this->auftrag}', ".((is_numeric($this->auftragid)) ? $this->auftragid : '0').", '{$this->freitext}', '{$this->status}', ".((is_numeric($this->adresse)) ? $this->adresse : '0').", ".((is_numeric($this->mitarbeiter)) ? $this->mitarbeiter : '0').", '{$this->name}', '{$this->abteilung}', '{$this->unterabteilung}', '{$this->strasse}', '{$this->adresszusatz}', '{$this->ansprechpartner}', '{$this->plz}', '{$this->ort}', '{$this->land}', '{$this->ustid}', '{$this->email}', '{$this->telefon}', '{$this->telefax}', '{$this->betreff}', '{$this->kundennummer}', '{$this->versandart}', '{$this->versand}', ".((is_numeric($this->firma)) ? $this->firma : '0').", ".((is_numeric($this->versendet)) ? $this->versendet : '0').", ".(($this->versendet_am=='' || $this->versendet_am=='--') ? 'NOW()' : "'".$this->versendet_am."'").", '{$this->versendet_per}', '{$this->versendet_durch}', ".((is_numeric($this->inbearbeitung_user)) ? $this->inbearbeitung_user : '0').", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->ohne_briefpapier)) ? $this->ohne_briefpapier : '0').", ".((is_numeric($this->schreibschutz)) ? $this->schreibschutz : '0').")"; 
