...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO auftrag_position (id,auftrag,artikel,projekt,bezeichnung,beschreibung,internerkommentar,nummer,menge,preis,waehrung,lieferdatum,vpe,sort,status,umsatzsteuer,bemerkung,geliefert,geliefert_menge,explodiert,explodiert_parent,logdatei,rabatt,nachbestelltexternereinkauf)
      VALUES(DEFAULT, ".((is_numeric($this->auftrag)) ? $this->auftrag : '0').", ".((is_numeric($this->artikel)) ? $this->artikel : '0').", ".((is_numeric($this->projekt)) ? $this->projekt : '0').", '{$this->bezeichnung}', '{$this->beschreibung}', '{$this->internerkommentar}', '{$this->nummer}', '{$this->menge}', ".((is_numeric($this->preis)) ? $this->preis : '0').", '{$this->waehrung}', ".(($this->lieferdatum=='' || $this->lieferdatum=='--') ? 'NOW()' : "'".$this->lieferdatum."'").", '{$this->vpe}', ".((is_numeric($this->sort)) ? $this->sort : '0').", '{$this->status}', '{$this->umsatzsteuer}', '{$this->bemerkung}', ".((is_numeric($this->geliefert)) ? $this->geliefert : '0').", ".((is_numeric($this->geliefert_menge)) ? $this->geliefert_menge : '0').", ".((is_numeric($this->explodiert)) ? $this->explodiert : '0').", ".((is_numeric($this->explodiert_parent)) ? $this->explodiert_parent : '0').", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->rabatt)) ? $this->rabatt : '0').", ".((is_numeric($this->nachbestelltexternereinkauf)) ? $this->nachbestelltexternereinkauf : '0').")"; 
