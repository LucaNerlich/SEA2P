...
class ObjGenArbeitsnachweis_Position
{
  ...
  public function ObjGenArbeitsnachweis_Position($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO arbeitsnachweis_position (id,arbeitsnachweis,artikel,projekt,bezeichnung,beschreibung,ort,internerkommentar,nummer,verrechnungsart,menge,arbeitspaket,datum,von,bis,sort,status,bemerkung,abgerechnet,logdatei,adresse)
      VALUES(DEFAULT, ".((is_numeric($this->arbeitsnachweis)) ? $this->arbeitsnachweis : '0').", '{$this->artikel}', ".((is_numeric($this->projekt)) ? $this->projekt : '0').", '{$this->bezeichnung}', '{$this->beschreibung}', '{$this->ort}', '{$this->internerkommentar}', '{$this->nummer}', '{$this->verrechnungsart}', '{$this->menge}', ".((is_numeric($this->arbeitspaket)) ? $this->arbeitspaket : '0').", ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", '{$this->von}', '{$this->bis}', ".((is_numeric($this->sort)) ? $this->sort : '0').", '{$this->status}', '{$this->bemerkung}', ".((is_numeric($this->abgerechnet)) ? $this->abgerechnet : '0').", ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").", ".((is_numeric($this->adresse)) ? $this->adresse : '0').")"; 
