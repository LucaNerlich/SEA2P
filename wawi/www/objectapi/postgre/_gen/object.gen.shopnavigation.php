...
class ObjGenShopnavigation
{
  ...
  public function ObjGenShopnavigation($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO shopnavigation (id,bezeichnung,position,parent,bezeichnung_en,plugin,pluginparameter,shop,target)
      VALUES(DEFAULT, '{$this->bezeichnung}', ".((is_numeric($this->position)) ? $this->position : '0').", ".((is_numeric($this->parent)) ? $this->parent : '0').", '{$this->bezeichnung_en}', '{$this->plugin}', '{$this->pluginparameter}', ".((is_numeric($this->shop)) ? $this->shop : '0').", '{$this->target}')"; 
