...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO emailbackup_mails (id,webmail,subject,sender,action,action_html,empfang,anhang,gelesen,checksum,adresse,spam,antworten,phpobj,flattenedparts,attachment)
      VALUES(DEFAULT, ".((is_numeric($this->webmail)) ? $this->webmail : '0').", '{$this->subject}', '{$this->sender}', '{$this->action}', '{$this->action_html}', ".(($this->empfang=='' || $this->empfang=='--') ? 'NOW()' : "'".$this->empfang."'").", '{$this->anhang}', ".((is_numeric($this->gelesen)) ? $this->gelesen : '0').", '{$this->checksum}', ".((is_numeric($this->adresse)) ? $this->adresse : '0').", ".((is_numeric($this->spam)) ? $this->spam : '0').", ".((is_numeric($this->antworten)) ? $this->antworten : '0').", '{$this->phpobj}', '{$this->flattenedparts}', '{$this->attachment}')"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE emailbackup_mails SET
      webmail=".((is_numeric($this->webmail)) ? $this->webmail : '0').",
      subject='{$this->subject}',
      sender='{$this->sender}',
      action='{$this->action}',
      action_html='{$this->action_html}',
      empfang=".(($this->empfang=='' || $this->empfang=='--') ? 'NOW()' : "'".$this->empfang."'").",
      anhang='{$this->anhang}',
      gelesen=".((is_numeric($this->gelesen)) ? $this->gelesen : '0').",
      checksum='{$this->checksum}',
      adresse=".((is_numeric($this->adresse)) ? $this->adresse : '0').",
      spam=".((is_numeric($this->spam)) ? $this->spam : '0').",
      antworten=".((is_numeric($this->antworten)) ? $this->antworten : '0').",
      phpobj='{$this->phpobj}',
