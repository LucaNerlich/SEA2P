...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO abrechnungsartikel (id,sort,artikel,bezeichnung,nummer,menge,preis,steuerklasse,rabatt,abgerechnet,startdatum,lieferdatum,abgerechnetbis,wiederholend,zahlzyklus,abgrechnetam,rechnung,projekt,adresse,status,bemerkung,logdatei)
      VALUES(DEFAULT, ".((is_numeric($this->sort)) ? $this->sort : '0').", ".((is_numeric($this->artikel)) ? $this->artikel : '0').", '{$this->bezeichnung}', '{$this->nummer}', '{$this->menge}', ".((is_numeric($this->preis)) ? $this->preis : '0').", '{$this->steuerklasse}', '{$this->rabatt}', ".((is_numeric($this->abgerechnet)) ? $this->abgerechnet : '0').", ".(($this->startdatum=='' || $this->startdatum=='--') ? 'NOW()' : "'".$this->startdatum."'").", ".(($this->lieferdatum=='' || $this->lieferdatum=='--') ? 'NOW()' : "'".$this->lieferdatum."'").", ".(($this->abgerechnetbis=='' || $this->abgerechnetbis=='--') ? 'NOW()' : "'".$this->abgerechnetbis."'").", ".((is_numeric($this->wiederholend)) ? $this->wiederholend : '0').", ".((is_numeric($this->zahlzyklus)) ? $this->zahlzyklus : '0').", ".(($this->abgrechnetam=='' || $this->abgrechnetam=='--') ? 'NOW()' : "'".$this->abgrechnetam."'").", ".((is_numeric($this->rechnung)) ? $this->rechnung : '0').", ".((is_numeric($this->projekt)) ? $this->projekt : '0').", ".((is_numeric($this->adresse)) ? $this->adresse : '0').", '{$this->status}', '{$this->bemerkung}', ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE abrechnungsartikel SET
      ...
      abgerechnetbis=".(($this->abgerechnetbis=='' || $this->abgerechnetbis=='--') ? 'NOW()' : "'".$this->abgerechnetbis."'").",
      wiederholend=".((is_numeric($this->wiederholend)) ? $this->wiederholend : '0').",
      zahlzyklus=".((is_numeric($this->zahlzyklus)) ? $this->zahlzyklus : '0').",
      abgrechnetam=".(($this->abgrechnetam=='' || $this->abgrechnetam=='--') ? 'NOW()' : "'".$this->abgrechnetam."'").",
