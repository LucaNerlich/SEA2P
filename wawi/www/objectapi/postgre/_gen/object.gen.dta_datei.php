...
class ObjGenDta_Datei
{
  ...
  public function ObjGenDta_Datei($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO dta_datei (id,bezeichnung,bearbeiter,inhalt,datum,status,art,firma)
      VALUES(DEFAULT, '{$this->bezeichnung}', '{$this->bearbeiter}', '{$this->inhalt}', ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", '{$this->status}', '{$this->art}', ".((is_numeric($this->firma)) ? $this->firma : '0').")"; 
