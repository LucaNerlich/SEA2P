...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO dta (id,adresse,datum,name,konto,blz,betrag,vz1,vz2,vz3,lastschrift,gutschrift,kontointern,datei,status,firma)
      VALUES(DEFAULT, ".((is_numeric($this->adresse)) ? $this->adresse : '0').", ".(($this->datum=='' || $this->datum=='--') ? 'NOW()' : "'".$this->datum."'").", '{$this->name}', ".((is_numeric($this->konto)) ? $this->konto : '0').", ".((is_numeric($this->blz)) ? $this->blz : '0').", ".((is_numeric($this->betrag)) ? $this->betrag : '0').", '{$this->vz1}', '{$this->vz2}', '{$this->vz3}', ".((is_numeric($this->lastschrift)) ? $this->lastschrift : '0').", ".((is_numeric($this->gutschrift)) ? $this->gutschrift : '0').", ".((is_numeric($this->kontointern)) ? $this->kontointern : '0').", ".((is_numeric($this->datei)) ? $this->datei : '0').", '{$this->status}', ".((is_numeric($this->firma)) ? $this->firma : '0').")"; 
  ...
  public function Update()
  {
    ...
    $sql = "UPDATE dta SET
      ...
      blz=".((is_numeric($this->blz)) ? $this->blz : '0').",
      betrag=".((is_numeric($this->betrag)) ? $this->betrag : '0').",
      vz1='{$this->vz1}',
      vz2='{$this->vz2}',
      vz3='{$this->vz3}',
      lastschrift=".((is_numeric($this->lastschrift)) ? $this->lastschrift : '0').",
      gutschrift=".((is_numeric($this->gutschrift)) ? $this->gutschrift : '0').",
      kontointern=".((is_numeric($this->kontointern)) ? $this->kontointern : '0').",
      datei=".((is_numeric($this->datei)) ? $this->datei : '0').",
      status='{$this->status}',
