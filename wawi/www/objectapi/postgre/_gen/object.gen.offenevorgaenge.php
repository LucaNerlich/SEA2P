...
class ObjGenOffenevorgaenge
{
  ...
  public function ObjGenOffenevorgaenge($app)
  {
    $this->app = $app;
  }
...
$result = $result[0];
  ...
  public function Create()
  {
    $sql = "INSERT INTO offenevorgaenge (id,adresse,titel,href,beschriftung,linkremove,logdatei)
      VALUES(DEFAULT, ".((is_numeric($this->adresse)) ? $this->adresse : '0').", '{$this->titel}', '{$this->href}', '{$this->beschriftung}', '{$this->linkremove}', ".(($this->logdatei=='' || $this->logdatei=='--') ? 'NOW()' : "'".$this->logdatei."'").")"; 
