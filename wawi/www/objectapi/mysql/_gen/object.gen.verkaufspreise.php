...
$result = $result[0];
  ...
  {
    $sql = "INSERT INTO verkaufspreise (id,artikel,objekt,projekt,adresse,preis,waehrung,ab_menge,vpe,vpe_menge,angelegt_am,gueltig_bis,bemerkung,bearbeiter,logdatei,firma,geloescht,kundenartikelnummer,art,gruppe)
      VALUES('','{$this->artikel}','{$this->objekt}','{$this->projekt}','{$this->adresse}','{$this->preis}','{$this->waehrung}','{$this->ab_menge}','{$this->vpe}','{$this->vpe_menge}','{$this->angelegt_am}','{$this->gueltig_bis}','{$this->bemerkung}','{$this->bearbeiter}','{$this->logdatei}','{$this->firma}','{$this->geloescht}','{$this->kundenartikelnummer}','{$this->art}','{$this->gruppe}')"; 
