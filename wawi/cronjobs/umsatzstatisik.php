...
$all = array('rechnung','gutschrift','auftrag','angebot');
foreach($all as $key)
{
	$result = $app->DB->SelectArr("SELECT id FROM $key WHERE deckungsbeitragcalc!=1");
	for($i=0;$i<count($result);$i++)
	{
		echo "$key Wert ".$result[$i]['id']."\r\n";
		$app->erp->BerechneDeckungsbeitrag($result[$i]['id'],$key);
	}
}
echo "done\r\n";
