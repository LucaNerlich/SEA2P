...
$WAWISION['host']="update.embedded-projects.net";
$WAWISION['port']="443";
...
class UpgradeClient
{
    ...
    $parameter['version']=@$this->conf['version'];
		...
		//$result = unserialize(gzuncompress(base64_decode($result)));
	...
	{
		global $erp;
                $auth['serial']=$erp->Firmendaten("lizenz");//$this->conf['serial'];
                $auth['authkey']=$erp->Firmendaten("schluessel");//$this->conf['authkey'];
		...
		$client->post('/upgradesystem.php', array( "auth" => $auth, "parameter"=>$parameter,"command"=>"$command" ));
