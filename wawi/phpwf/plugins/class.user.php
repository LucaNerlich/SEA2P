...
class User 
{
  ...
  function GetID()
  { 
		if($_COOKIE["CH42SESSION"]!="")$tmp = $_COOKIE["CH42SESSION"]; else $tmp = session_id();
                return $this->app->DB->Select("SELECT user_id FROM useronline WHERE sessionid='".$tmp."' AND login ='1'");

		if($this->app->Conf->WFdbType=="postgre")
    {
      $result =  $this->app->DB->Select("SELECT user_id FROM useronline WHERE sessionid='".$tmp."' AND login='1'");
...
//        AND ip='".$_SERVER[REMOTE_ADDR]."' AND login='1'");
  ...
  function SetParameter($index,$value)
  {
		...
		if($index!="" && isset($value))
		{
			...
			if($this->app->Conf->WFdbType=="postgre")
			$this->app->DB->Update("UPDATE \"user\" SET settings='$settings' WHERE id='$id'");
			else
