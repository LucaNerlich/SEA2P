...
class FormActionHandler
{
  ...
  */
  ...
  function ActionAfterExecute()
  {
    ...
    if($action=="create") { 
			$msg = base64_encode("<div class=\"error2\">Die Daten wurden erfolgreich gespeichert!</div>");
      header("Location: ".$this->app->http."://".$_SERVER['HTTP_HOST']
	.dirname($_SERVER['PHP_SELF'])
	."/index.php?module=$module&action=edit&id=".$this->pkvalue."&msg=".$msg);
			exit;
    } 
    else if($action=="edit") 
    { 
	$msg = base64_encode("<div class=\"error2\">Die Daten wurden erfolgreich gespeichert!</div>");
      header("Location: ".$this->app->http."://".$_SERVER['HTTP_HOST']
	.dirname($_SERVER['PHP_SELF'])
	."/index.php?module=$module&action=edit&id=".$id."&msg=".$msg);
			exit;
    } 
  ...
  function PrintForm()
  {
    foreach($this->HTMLList as $name=>$entry){
      ...
      {	
				...
				$entry->htmlobject->value = $arr[1]->$arr[2](0,$entry->htmlobject->value,0);
      }
  ...
  function Edit()
  {
	...
	foreach($this->HTMLList as $name=>$entry){ 	
	  ...
	  if(method_exists($tmp,$methodname)){
	    ...
	    { 
	      // aufruf der replacecallback funktion
	      $value = $arr[1]->$arr[2](1,$value,1); // wenn speichern dann 1 bei uebernehmen = 0
	    }
	...
	$tmp->Update();
			...
			$this->app->erp->EventAPIAdd("Event".ucfirst($this->table)."Edit",$tmp->GetId(),$this->table,"edit",$kommentar="");
//     $this->app->Tpl->Add(MESSAGE,'<div class="error2">Die Daten wurden erfolgreich gespeichert!</div>');
  ...
  function Copy()
  {
    //if($this->FormhandlerEvent()) {
      ...
      $tmp->id="";
      $tmp->Create();
			$this->app->erp->EventAPIAdd("Event".ucfirst($this->table)."Create",$tmp->GetId(),$this->table,"create",$kommentar="");
      // passt und speichern
      $this->ActionAfterExecute();
    //}
  }
...
class FormHandler
{
  ...
  function FormToDatabase($table,$extfield,$extvalue)
  {
    ...
    $this->app->erp->EventAPIAdd("Event".ucfirst($table)."Edit",$id,$table,"edit",$kommentar="");
    return $id;
  }

  function ArrayUpdateDatabase($table,$id,$values)
  {
    //$this->app->DB->Insert("INSERT INTO $table ($extfield) VALUES ('$extvalue')");
